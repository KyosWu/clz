...
<article>
    ...
    <el-table ref="multipleTable" :data="userList" tooltip-effect="dark" stripe border>
        ...
        <el-table-column show-overflow-tooltip v-if="!item.hidden" v-for="(item, index) in headerOptions" :key="list" :label="item.label" :prop="item.prop" :header-align="item.headerAlign" :align="item.align" :min-width="item.minWidth || 150">
            <template slot-scope="scope">
                <div v-if="scope.column.property === 'roles'">
                    <el-tag
                        ...
                        type="primary"
                        close-transition
                        v-for="(tag, index) in scope.row.roles" :key="list">{{tag}}</el-tag>
                </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            increment (index) {
                return list+1+((this.pageindex-1)*this.pagesize)
            },
            close () {
                this.editShow = false;
                this.getUserList()
